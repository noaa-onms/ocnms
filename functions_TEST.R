...
#adding in specific info from OCNMS master spreadsheet:
gsheet_ocnms    <- "https://docs.google.com/spreadsheets/d/1C5YAp77WcnblHoIRwA_rloAagkLn0gDcJCda8E8Efu4/export?format=xlsx"
# modals_xlsx    <- glue("{gsheet_pfx}&sheet=modals")
# scenes_xlsx    <- glue("{gsheet_pfx}&sheet=scenes")
# figures_xlsx <- glue("{gsheet_pfx}&sheet=figures")
# glossary_xlsx  <- glue("{gsheet_pfx}&sheet=glossary")

#NOTE: output changed to .xlsx to retain [meta]data across sheets (".../export?format=csv" dropped all but first sheet in workbook)

#Do we have this for OCNMS?
sites_csv <- here("ocnms/data/nms_sites.csv")
sites_geo <- here("ocnms/data/nms_sites.geojson")

get_sheet_csv <- function(sheet){
  glue("{gsheet_ocnms}&sheet={sheet}")
}
...
get_sheet <- function(sheet, redo = F){
  ...
  if (!exists(sheet, envir = globalenv()) | redo){
    ...
    d <- get_sheet_xlsx(sheet) %>% 
      read_xlsx(col_types = cols()) %>% 
      select(-starts_with("X"))
    msg <- glue("get_sheet(): read_xlsx(URL), assign to variable '{sheet}'\n  URL: {get_sheet_xlsx(sheet)}", .trim = F)
...
#I think this should be correct, based on the OCNMS master spreadsheet
modals    <- get_sheet("modals")
scenes    <- get_sheet("scenes")
figures <- get_sheet("figures")
glossary  <- get_sheet("glossary")

get_nms_ply <- function(nms, dir_pfx = here("ocnms")){
  ...
  if (!file.exists(nms_shp)){
    ...
    nms_zip <- here::here(glue::glue("ocnms/data/{nms}.zip"))
    shp_dir <- here::here("ocnms/data/shp")
...
create_sanctuary_pages <- function(){
  ...
  site_rmd <- function(code, name){
    ...
    lns <- glue::glue('
      ...
      ```
      ```{r}
      source(here::here("ocnms/functions.R"))
...
get_modal_tab_content <- function(tab_name, modal_title = NULL, rmd = NULL){
  ...
  modal_tbl <- get_modal_tbl(modal_title = title, rmd = rmd)
  ...
  imgs <- modals %>% 
    ...
    mutate(
      gdrive_id = str_replace(
        ...
        "https://drive.google.com/drive/folders/1NGlP1H8Rq9Hr2EfLIi3N_tx6NBAPMGQt?usp=sharing", 
        "\\1"), #this is the OCNMS master spreadsheet
      gdrive_dl = glue("https://drive.google.com/uc?id={gdrive_id}&export=download"),#is this where the ONCMS figures link should go?
      path_0    = here(glue("ocnms/images/{filename}")), #this probably needs to get changed, gsheet or file directory?
  ...
  snds <- modals %>% 
    ...
    mutate(
      gdrive_id = str_replace(
        gdrive_shareable_link, 
        "https://drive.google.com/drive/folders/1NGlP1H8Rq9Hr2EfLIi3N_tx6NBAPMGQt?usp=sharing", 
        "\\1"), #again, the OCNMS master spreadsheet 
      gdrive_dl = glue("https://drive.google.com/uc?id={gdrive_id}&export=download"), #same q as above
...
glink_to_gid <- function(glink){
  ...
  # glink = "https://drive.google.com/drive/folders/1NGlP1H8Rq9Hr2EfLIi3N_tx6NBAPMGQt?usp=sharing" 
  str_replace(
    glink, 
...
modal_title_to_html_path <- function(sanctuary_code, modal_title, pfx = here("ocnms/modals")){
...
render_sanctuary <- function(code, name, type, ...){
  in_rmd  <- here("ocnms/_sanctuary_template.Rmd")
  out_htm <- here(glue("ocnms/s_{code}.html"))
  ...
  render(
    ...
    params      = list(
      ...
      csv       = "https://docs.google.com/spreadsheets/d/1C5YAp77WcnblHoIRwA_rloAagkLn0gDcJCda8E8Efu4/edit#gid=1778111652/tq?tqx=out:csv&sheet=modals",
      svg       = glue("svg/{code}.svg")))
}
  #csv = now has specific link to "modals" sheet in OCNMS master spreadshet
render_modal <- function(sanctuary_code, modal_title, modal_html, rmd = here("ocnms/modals/_modal_template.Rmd")){
...
render_page <- function(rmd){
  render(rmd, html_document(
    ...
    self_contained=F, lib_dir = here("ocnms/modals/modal_libs"), 
    mathjax = NULL))
}
...
# no longer need? import_sounds <- function(redo = T){
  sounds_csv <- here("ocnms/data/sounds.csv")
...
# no longer need? import_stories <- function(redo = T){
  stories_csv <- here("ocnms/data/stories.csv")
...
# no longer need? header2anchor <- function(header, pfx="stories.html"){
...
# no longer need? story_grid_item <- function(title, img_rel, story_link, ...){
  if(is.na(story_link)){
    glue("
      <div class='grid-item'>
...
# no longer need? story_card <- function(title, img_rel, story_link, ...){
  a <- ""
  if(!is.na(story_link))
...
# no longer need? update_sounds_menu <- function(){
  
  tbl_sounds <- import_sounds()
  
  site <- read_yaml(here("ocnms/_site.yml"))
  ...
  write_yaml(site, here("ocnms/_site.yml"))
}

# no longer need? update_stories_menu <- function(){
...
# Started to update, no longer need? gdrive2path <- function(gdrive_shareable_link, get_relative_path = T, relative_pfx = "../", redo = F, skip_spectrogram = F){
  
  # gdrive_shareable_link <- "https://docs.google.com/spreadsheets/d/1C5YAp77WcnblHoIRwA_rloAagkLn0gDcJCda8E8Efu4/edit?usp=sharing"
  ...
  path          <- here(glue("ocnms/files/{fname_ok}"))
...
update_modal_imgs_snds <- function(modals_csv = modals_csv){
  ...
  imgs <- modals %>% 
    ...
    mutate(
      ...
      path_0    = here(glue("ocnms/images/{filename}")),
      path_w    = glue("{path_ext_remove(path_0)}_w{image_width_inches}in.{path_ext(path_0)}"),
      path_r    = glue("../images/{basename(path_w)}"))
